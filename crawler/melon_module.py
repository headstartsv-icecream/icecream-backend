...
def get_data(html_source):
    ...
    comments = soup.select(
        'div.wrap_cntt.d_cmtpgn_cmt_cont_wrapper > div > div')
    dates = soup.select(
        'div.wrap_cntt.d_cmtpgn_cmt_cont_wrapper > div > span.date')
...
def melon_crawling(title, singer):
    ...
    driver.get("https://www.melon.com/search/total/index.htm?q=" +
               str(search_word(title, singer)))

    driver.find_element_by_xpath(
        '//*[@id="divCollection"]/ul/li[3]/a/span').click()  # 검색 항목에서 '곡'으로 바꿈
    ...
    driver.find_element_by_xpath(
        '//*[@id="d_cmtpgn_cmt_count_wrapper"]/ul/li[2]/a').click()  # 추천순으로 변경
    # html을 온전히 불러오기 위한 대기
    wait.until(
        visible((By.XPATH, '//*[@id="d_cmtpgn_cmt_list_wrapper"]/ul/li[1]')))
    ...
    # 반복 횟수 설정
    soup = BeautifulSoup(html_source, 'lxml')
    number_of_comments = int(soup.select_one(
        '#d_cmtpgn_cmt_count_wrapper > div > strong > span').get_text().replace(',', ''))
    ...
    for i in range(1, repeat_num+1):
        driver.find_element_by_xpath(
            '//*[@id="d_cmtpgn_paginate_wrapper"]/span/a[%d]' % i).click()  # 댓글이동
    ...
    pd_data = {"music": title, "artist": singer, "userName": str_melon_userIDs,
               "content": str_melon_comments, "writingDate": str_melon_date, "source": 'melon'}
