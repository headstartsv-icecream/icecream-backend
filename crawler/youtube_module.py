...
def find_comment(html_source, title, singer):
    ...
    for i in range(len(youtube_user_IDs)):
        ...
        str_youtube_date.append(
            str(youtube_date[i].text).replace('(edited)', ''))

    print("youtube 가져온 댓글 갯수: ", len(str_youtube_userIDs))

    pd_data = {"music": title, "artist": singer, "userName": str_youtube_userIDs,
               "content": str_youtube_comments, "writingDate": str_youtube_date, "source": 'youtube'}
...
def search_music(title, singer):
    ...
    try:
        wait.until(
            visible((By.XPATH, '//*[@id="merch-shelf"]/ytd-merch-shelf-renderer')))
    except:
        merch_shelf = False
        print("상품 없음")
    if(merch_shelf):
    ...
    for i in range(1, 6):
        ...
        try:
            wait.until(element(
                (By.XPATH, '//*[@id="contents"]/ytd-comment-thread-renderer[%d]' % num)))
        except:
            break
        else:
